...
- 文体は敬体（です・ます）。手順は簡潔な命令形も可。
- フレンドリー × 実務的。誇張・煽り・過度な絵文字は避け、短文・能動・具体で書く（成果語を入れる）。
- 2 つのテンプレを使用: ツール導入（価値 → 3 手 → 注意 → 入門リンク）/ 入門記事（導入 → 定義 → 仕組み → つまずき → 実例 → セキュリティ/運用 → 手を動かす → 3 分クイズ → 関連リンク）。
- ブログ一覧の要約は「課題 → 得られること（名詞句 2 つまで）」で 1〜2 文。
...
- **目的**: トラフィック実態に合わせ「Tools > Primers > Archive」の序列で構成
- **URL 維持**: 既存 URL は変更せず、visibility で見せ方のみ変更
- **3 層構造**:
  - `primer`: ツール入門記事。Blog 一覧・Sitemap に表示
  - `archive`: 低パフォーマンス記事。`/blog/archive`（noindex）のみ表示
  - `hidden`: 完全非表示
- **相互リンク**: ツール ⇄ Primer 双方向リンクを明示
- **KPI**: ツール別 UU・滞在・遷移率を週次監視

詳細は `PROJECT_SPEC.md` の「Tools-First 移行方針」章を参照。
