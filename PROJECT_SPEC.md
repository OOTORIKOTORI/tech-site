...
- frontmatter フェンスガード: すべてのブログ記事が `---\n` で開始すること（`---` から始まること）を静的チェックで担保。
...
（この節は冒頭の「ブログ詳細の選択ロジック（正準）」に集約済みのため、重複する詳細は削除）
...
## ✍️ Writing Style（文体・トーンの正準）

- 文体: 基本は敬体（です・ます）。手順や箇条の操作説明は簡潔な命令形（〜する/クリックする）を許可。
- トーン: フレンドリー × 実務的。温かいがキレのある表現を心がけ、誇張・煽り・過度な絵文字は避ける。
- 文章作法: “読者が今できる行動”を主語に据え、能動・具体・短文で書く（例: 「ボタンを押して実行する」）。

### ツール導入の型（固定）

1. 価値訴求（成果語を含む 1 文）
2. 使い方（3 手以内の番号ステップ）
3. 注意（例: ブラウザ内処理／外部取得のレート・プライバシーの留意点）
4. 入門記事リンク（後述の相互リンク運用に従う）

### 入門記事の見出し型（固定順）

「導入 → 定義 → 仕組み → よくあるつまずき → 実例 → セキュリティ/運用 → 手を動かす → 3 分クイズ → 関連リンク」

### ブログ一覧の要約フォーマット

カード要約は「課題 → 得られること（名詞句 2 つまで）」で 1〜2 文に収める。
