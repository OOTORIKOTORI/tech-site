...
- **関連記事タグの規則**: 入門記事の frontmatter に **`tool:<tool-id>`** を必ず付与する（RelatedList は `/blog/**` のみ・最大 3 件・`_archive`除外・`body`必須）。
- **相互リンク方針**: ツール側は導入直下に「入門記事へのリンク」を置く。入門記事側は冒頭または末尾に「関連ツール: /tools/<id>」の行を置く。
...
- **相互導線**: ツール詳細ページ末尾に `RelatedList` コンポーネントを配置し、tags ベースで関連記事3件を表示。
 - **ツール追加のルール**: 完全クライアント完結（アップロード不要）・a11y（ラベル/aria-live）・コピー/ダウンロード操作の既定を満たすこと。
...
### プロンプト運用の原則（追記）
- プロンプトは **ダウンロード可能な .md ファイル** で共有する
- プロンプトは **日本語** で書く
- **基本は「直接修正＆保存（直編集）」**。直編集に失敗した場合に限り、完成版の**ファイル全文**を1つのコードブロックで出力する
- 巨大テキストの貼付は避け、DLファイル運用を原則とする

### Pre-push 自動修復の方針
- 次の順で品質ゲートを実行し、失敗したら **最小差分で直し再実行** してグリーンにする
  `pnpm -s typecheck && pnpm -s lint && pnpm -s test -- --run && pnpm -s build && pnpm -s run ci:guards`
- `plugins/ads.ts`：`#app` から import しない／`defineNuxtPlugin` 未定義でも落ちないローカル登録関数でフォールバック
