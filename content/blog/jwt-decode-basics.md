...
「API のログインで JWT トークンが出てきたけど、何か分からない…」「デコードと検証の違いが曖昧で誤用しそう」そんな悩みはありませんか？
この記事では、JWT の基本構造とデコード・検証の違い、安全な運用ポイントを短時間で解説します。
まずは関連ツール「JWT Decoder」で実際に試してみましょう。

---

## 1. 基本の構造（定義と構文）

JWT（JSON Web Token）は、Header・Payload・Signature の 3 部構成です。
Header はアルゴリズムやトークンタイプ、Payload はユーザー情報や有効期限、Signature は改ざん検知に使います。
...
デコードは誰でもできます（Base64URL デコード）。検証は正しい鍵で署名の整合を確認する必要があります。
デコードだけでは安全性は保証されません。必ず検証も行いましょう。
...
1. JWT Decoder でトークンをデコードします
2. 検証タブで署名検証を行います
3. exp/nbf/iat の期限バッジを確認します
