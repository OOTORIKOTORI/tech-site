...
import { encodeBase64Url, findJwksRsaKeyByKid, buildRsaPemFromModExp, verifyJwt } from '../../utils/jwt'
...
describe('JWKS flow (kid -> n/e -> PEM -> verify)', () => {
  it('RS256: JWKS の n/e から PEM を組み立てて検証できる', async () => {
    ...
    const sigBuf = await crypto.subtle.sign('RSASSA-PKCS1-v1_5', privateKey, enc.encode(`${headerPart}.${payloadPart}`))
    const token = `${headerPart}.${payloadPart}.${encodeBase64Url(new Uint8Array(sigBuf))}`

    // 6) PEM 経由で verifyJwt
    const res = await verifyJwt(token, { expectedAlg: 'RS256', key: pem, currentTimeSec: 1700000000 })
